...
// ===== Función para eliminar en cascada =====
...
function renderizarComentario(doc, contenedor, nivel = 0) {
  try {
    const data = doc.data();
    const id = doc.id;
    const texto = data.texto || '📝 Mensaje sin texto';
    ...
    if (data.fecha && data.fecha.toDate) {
      ...
      fechaMostrar = new Date().toLocaleString('es-MX', {
        day: '2-digit',
        month: 'long',
        year: 'numeric',
        hour: '2-digit',
    ...
    btnEnviarRespuesta.addEventListener('click', async () => {
      const textoResp = textareaRespuesta.value.trim();
      if (textoResp === '') return;
      try {
    ...
    hijos.forEach(hijo => {
      ...
      renderizarComentario(hijoSimulado, respuestasContainer, nivel + 1);
    });
  } catch (error) {
    ...
    .onSnapshot((snapshot) => {
      ...
      docs.forEach(({ id, data }) => {
        ...
        if (parentId === null || parentId === '') {
          roots.push({ id, data });
          console.log(`✅ Raíz: ${data.texto || '(sin texto)'}`);
        } else {
          if (comentariosMap.has(parentId)) {
            comentariosMap.get(parentId).push({ id, data });
            console.log(`↳ Respuesta de ${parentId}: ${data.texto || '(sin texto)'}`);
          } else {
            roots.push({ id, data });
            console.log(`⚠️ Huérfano (padre ${parentId} no existe) -> raíz: ${data.texto || '(sin texto)'}`);
          }
        }
      });

      // Ordenar hijos por fecha ascendente (para mantener el hilo)
      ...
      console.log(`📌 Total de raíces: ${roots.length}`);
...
// ============================================
// 10. CARRUSEL HORIZONTAL AUTOMÁTICO
// ============================================
...
// Siguiente slide (con reinicio de autoplay)
function siguienteSlide() {
  irASlide(currentIndex + 1);
  reiniciarAutoplay();
}

// Anterior slide (con reinicio de autoplay)
...
function iniciarAutoplay() {
  ...
  autoPlayInterval = setInterval(siguienteSlide, AUTO_PLAY_DELAY);
}

// Reiniciar autoplay (por ejemplo, al hacer clic en botones)
...
// Detener autoplay (útil si el usuario interactúa)
...
// Pausar autoplay al hacer hover sobre el carrusel (opcional)
...
// También pausar al tocar en móviles (para evitar conflictos)
carruselContainer.addEventListener('touchstart', detenerAutoplay);
carruselContainer.addEventListener('touchend', () => {
  setTimeout(iniciarAutoplay, 3000); // reanuda después de 3s
});

// Iniciar autoplay al cargar la página
iniciarAutoplay();

// Ajustar el carrusel al redimensionar (por si cambia el ancho)
window.addEventListener('resize', () => {
  irASlide(currentIndex); // reaplica la posición
});

// El modal de imágenes sigue funcionando porque los eventos click
// ya están asignados a todas las imágenes .tarjeta img